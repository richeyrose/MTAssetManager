...
def draw_image(image_props, crop=(0, 0, 1, 1)):
    '''Draws an image at the given coordinates.

    Parameters
    ----------
    image_props : dict(
        x : int, x coordinate to begin drawing measured from lower left corner of area in px
        y : int y coordinate to begin drawing in px
        width : int, image width in px
        height : int, image height in px
        transparency : alpha transparency float, 0-1
        image : str, image name
    )
    crop : 4 tuple - UV tiling
    '''
    ...
    image = image_props['image']
    transparency = image_props['transparency']
